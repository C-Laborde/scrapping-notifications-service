...
def main(request):
    ...
    for url_details in root_urls:
        ...
        # If all the results are empty we can finish here. Only dump when
        # there has been a first result
        if games_played == 0:
            # TODO should I log this? It will be logged every 5 minutes before
            # the first results have been played..
        ...
        logger.info("There are results available. We compare them with " +
                    "previous results")
        # Now we should check for the same doc in the database and decide if
        # an email should be sent accordingly
        db.compare_and_send(document, url_id, weekend, logger, url,
                            email_service, teams)
